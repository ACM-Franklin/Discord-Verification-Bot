...
class Reactor(commands.Cog):
    """Reaction-based role assignment. React to a message to gain/lose a role."""
    ...
    @commands.guild_only()
    async def reactor_add(self, ctx: commands.Context, message_id: int, role_id: int, emoji: str) -> None:
    ...
    @commands.cooldown(1, 2, commands.BucketType.user)
    @commands.guild_only()
    async def reactor_get(self, ctx: commands.Context) -> None:
    ...
    @commands.cooldown(1, 2, commands.BucketType.user)
    @commands.guild_only()
    async def reactor_delete(self, ctx: commands.Context, message_id: int) -> None:
    ...
    @commands.cooldown(1, 2, commands.BucketType.user)
    @commands.guild_only()
    async def reactor_clear_all(self, ctx: commands.Context) -> None:
        """Clear all reactors."""
        await ctx.message.delete()
    ...
    async def on_raw_reaction_add(self, payload) -> None:
        await self._reaction_handle(payload, add_mode=True)

    @commands.Cog.listener("on_raw_reaction_remove")
    async def on_raw_reaction_remove(self, payload) -> None:
        await self._reaction_handle(payload, add_mode=False)
    ...
    async def on_raw_message_delete(self, payload) -> None:
        ...
        try:
            ...
            log.warning("Unable to load SQLAlchemy database — skipping reactor cleanup for deleted message.")

    async def _reaction_handle(self, payload, add_mode: bool) -> None:
        guild = self.bot.get_guild(payload.guild_id)
        if guild is None:
            return
        ...
        matching = [r for r in reactors if payload.message_id == r[1]]

        for reac in matching:
            re_msg_id = reac[1]
            re_role_id = reac[2]
            re_emoji = reac[3]

            if str(payload.emoji) == re_emoji:
                role = guild.get_role(re_role_id)
                if role is None:
                    log.warning("Role %s not found in guild %s — skipping.", re_role_id, guild.id)
                    continue
                ...
                try:
                    ...
                    log.exception("Failed to modify role %s for user %s", role.name, user)
